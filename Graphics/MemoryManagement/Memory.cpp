...
Memory::Memory(const GraphicsContext& instance, const Device& device,
...
bool Memory::bindBuffer(const GraphicsContext& instance, const Device& device, const Buffer& buffer, size_t offset) {
    if (!m_initialized)
        return false;
...
bool Memory::bindImage(const GraphicsContext& instance, const Device& device, const Image& image, size_t offset /*= 0*/)
{
    if (!m_initialized)
        return false;
