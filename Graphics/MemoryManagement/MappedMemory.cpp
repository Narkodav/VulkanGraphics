...
MappedMemory::MappedMemory(const GraphicsContext& instance, const Device& device,
...
bool MappedMemory::bindBuffer(const GraphicsContext& instance, const Device& device, const Buffer& buffer, size_t offset) {
    if (!m_initialized)
        return false;
...
bool MappedMemory::bindImage(const GraphicsContext& instance, const Device& device,
