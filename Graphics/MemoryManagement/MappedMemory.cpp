...
namespace Graphics {
    ...
    MappedMemory::MappedMemory(const Context& instance, const Device& device,
    ...
    bool MappedMemory::bindBuffer(const Context& instance, const Device& device, const Buffer& buffer, size_t offset) {
        if (!m_initialized)
            return false;
    ...
    bool MappedMemory::bindImage(const Context& instance, const Device& device,
