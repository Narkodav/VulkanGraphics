...
public:
    ...
    void record(const GraphicsContext& instance, CommandBufferUsage::Flags flags = 0);
    void beginRenderPass(const GraphicsContext& instance, const RenderPass& renderPass, 
    ...
    void bindGraphicsPipeline(const GraphicsContext& instance, const GraphicsPipeline& pipeline);
    
    template<size_t bufferAmount>
    void bindVertexBuffers(const GraphicsContext& instance,
    ...
    void bindIndexBuffer(const GraphicsContext& instance,
        const Buffer& buffer, vk::DeviceSize offset);
    
    void bindDescriptorSets(const GraphicsContext& instance,
        const GraphicsPipeline& pipeline, const std::vector<DescriptorSetHandle>& descriptorSets,
        const std::vector<uint32_t>& dynamicOffsets = {});

    void setPipelineBarrier(const GraphicsContext& instance,
        vk::PipelineStageFlags srcStage, vk::PipelineStageFlags dstStage,
        Image& image, vk::ImageLayout newLayout,
        vk::AccessFlags srcAccess, vk::AccessFlags dstAccess);

    void transferBufferData(const GraphicsContext& instance, const Buffer& srcBuffer,
        const Buffer& dstBuffer, const CopyRegion& copyRegion);

    void transferImageData(const GraphicsContext& instance, const Buffer& srcBuffer,
        Image& dstImage, size_t offset = 0, vk::Offset3D imageOffset = { 0, 0, 0 });

    void setRenderView(const GraphicsContext& instance, const RenderRegion& canvas);
    void draw(const GraphicsContext& instance,
        size_t vertexCount, size_t instanceCount, size_t firstVertex, size_t firstInstance);
    void drawIndexed(const GraphicsContext& instance,
        size_t indexCount, size_t instanceCount, size_t firstIndex, size_t indexIncrement, size_t firstInstance);

    void endRenderPass(const GraphicsContext& instance);
    void stopRecord(const GraphicsContext& instance);
    void reset(const GraphicsContext& instance);
