...
public:
    ...
    Device(const GraphicsContext& instance, const Window& window) {
    ...
    bool reevaluateSwapChainSupportDetails(const GraphicsContext& instance, const Window& window) {
    ...
    static bool evaluateDevice(vk::PhysicalDevice device, const GraphicsContext& instance, const Window& window,
    ...
    void waitIdle(const GraphicsContext& instance) const {
        m_device.waitIdle(instance.getDispatchLoader());
    };

    vk::DeviceMemory allocateMemory(const GraphicsContext& instance,
        size_t requiredCapacity, uint32_t memoryType) const;

    uint32_t findMemoryType(const GraphicsContext& instance, 
        uint32_t typeFilter, vk::MemoryPropertyFlags properties) const;

    vk::Format findSupportedFormat(const GraphicsContext& instance, 
    const std::vector<vk::Format>& candidates,
        vk::ImageTiling tiling, vk::FormatFeatureFlags features) const;
private:
    void selectPhysicalDevice(const GraphicsContext& instance,
        const Window& window);
    void createLogicalDevice(const GraphicsContext& instance,
        const Window& window);
