...
private:
    ...
    //std::vector<std::unique_ptr<VertexDefinitionTemplate>> m_vertices;
    std::vector<vk::DescriptorSetLayoutBinding> m_descriptors;
    vk::DescriptorSetLayoutCreateInfo m_descriptorSetLayoutInfo;
    ...
    std::optional<vk::PipelineTessellationStateCreateInfo>>
        createShaderStages() {
            using Result = std::pair<std::vector<vk::PipelineShaderStageCreateInfo>,
                std::optional<vk::PipelineTessellationStateCreateInfo>>;
            Result result;
...
public:
        ...
        const DescriptorDefinitions<DescriptorDefs...>& descriptors)
    {
        ...
        m_descriptors = descriptors.enumerateDescriptors();
        m_descriptorSetLayoutInfo.sType = vk::StructureType::eDescriptorSetLayoutCreateInfo;
        m_descriptorSetLayoutInfo.bindingCount = static_cast<uint32_t>(m_descriptors.size());
        m_descriptorSetLayoutInfo.pBindings = m_descriptors.data();

        try {
            m_descriptorSetLayout = device.getDevice().
                createDescriptorSetLayout(m_descriptorSetLayoutInfo, nullptr, instance.getDispatchLoader());
        }
        ...
        pipelineLayoutInfo.pSetLayouts = &m_descriptorSetLayout;  // array of descriptor set layouts
        ...
        auto result = device.getDevice().createGraphicsPipeline(nullptr, pipelineInfo);
    ...
    GraphicsPipeline(GraphicsPipeline&& other) noexcept {
        ...
        m_descriptorSetLayout = std::exchange(other.m_descriptorSetLayout, nullptr);
        m_descriptorSetLayoutInfo = std::exchange(other.m_descriptorSetLayoutInfo, vk::DescriptorSetLayoutCreateInfo());
        m_descriptors = std::exchange(other.m_descriptors, {});
    ...
    void destroy(const EngineContext& instance, const Device& device) {
        ...
        device.getDevice().destroyDescriptorSetLayout(m_descriptorSetLayout, nullptr, instance.getDispatchLoader());
    ...
    const vk::DescriptorSetLayoutCreateInfo& getDescriptorSetLayoutCreateInfo() const
    {
