...
std::vector<vk::LayerProperties> EngineContext::getAvailableValidationLayers() {
    try {
        uint32_t layerCount;
...
std::vector<const char*> EngineContext::validateLayers(std::vector<vk::LayerProperties>& availible)
{
...
void EngineContext::checkLayerSupport()
{
...
std::vector<const char*> EngineContext::getRequiredExtensions()
{
    try {
...
std::vector<vk::ExtensionProperties> EngineContext::getAvailableExtensions()
{
    try {
...
void EngineContext::validateExtensions(
...
vk::ApplicationInfo EngineContext::getAppInfo(const std::string& engineName,
...
vk::InstanceCreateInfo EngineContext::getInstanceCreateInfo(vk::ApplicationInfo& appInfo,
    std::vector<const char*>& requiredExtensions,
...
vk::Instance EngineContext::getInstance(const std::string& engineName,
    const std::string& appName, Version& engineVersion, Version& appVersion,
    vk::DebugUtilsMessengerCreateInfoEXT* debugCreateInfo /*= nullptr*/)
{
...
vk::detail::DispatchLoaderDynamic EngineContext::getDispatchLoader(
...
vk::DebugUtilsMessengerCreateInfoEXT EngineContext::getDebugCreateInfo(
...
vk::DebugUtilsMessengerEXT EngineContext::getDebugMessenger(
    ...
    Version engineVersion, Version appVersion)
{
    ...
    m_enginePointer = new EngineContext*;
...
VKAPI_ATTR vk::Bool32 VKAPI_CALL EngineContext::static_debugCallback(
    ...
    void* pUserData) {
    EngineContext* context = *static_cast<EngineContext**>(pUserData);
    return context->debugCallback(messageSeverity, messageType,
        pCallbackData, pUserData);
}

VKAPI_ATTR vk::Bool32 VKAPI_CALL EngineContext::debugCallback(
    vk::DebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
    vk::DebugUtilsMessageTypeFlagsEXT messageType,
    const vk::DebugUtilsMessengerCallbackDataEXT* pCallbackData,
    void* pUserData)
